...
# There are two logical values in R, also called boolean values. They are
# TRUE and FALSE. In R you can construct logical expressions which will
...
# Let's look at how the AND operator works. There are two AND operators in
# R, `&` and `&&`. Both operators work similarly, if the right and left
# operands of AND are both TRUE the entire expression is TRUE, otherwise
# it is FALSE. For example, TRUE & TRUE evaluates to TRUE. Try typing
...
# The expressions: `6 != 10 && FALSE && 1 >= 2` or 
# `TRUE || 5 < 9.3 || FALSE` are perfectly normal to see.

# As you may recall, arithmetic has an order of operations and so do
# logical expressions. All AND operators are evaluated before OR operators.
# Let's look at an example of an ambiguous case. Type: 
5 > 8 || 6 != 8 && 4 > 3.9
# [1] TRUE

# Let's walk through the order of operations in the above case. First the
# left and right operands of the AND operator are evaluated. 6 is not equal
# 8, 4 is greater than 3.9, therefore both operands are TRUE so the
# resulting expression `TRUE && TRUE` evaluates to TRUE. Then the left
# operand of the OR operator is evaluated: 5 is not greater than 8 so the
# entire expression is reduced to FALSE || TRUE. Since the right operand of
...
# The function identical() will return TRUE if the two R objects passed to
# it as arguments are identical. Try out the identical() function by typing:
...
# TRUE, otherwise it will return FALSE. Try out the xor() function by
# typing: 
xor(5 == 6, !FALSE)
# [1] TRUE
...
# 5 == 5 and the second argument was unchanged then both arguments would
...
# integers called ints. Create this vector by typing: 
ints <- sample(10)
ints
# [1]  6  7 10  5  4  1  3  8  9  2
...
# indices of the vector that are TRUE. For example 
...
# more of the elements in the logical vector is TRUE. The all() function
